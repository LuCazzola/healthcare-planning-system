(define (domain healthcare_facility_5_base)
    (:requirements :strips :typing :durative-actions)

    (:types
        location carrier locatable - object
        med_unit worker_bot helper_bot box supply patient - locatable
    )
    ...
    (:predicates
        ...
        (worker_bot_is_free ?bot - worker_bot) ; bot is free (not carrying a box or helping a patient)
        (worker_bot_is_unlocked ?bot - worker_bot) ; predicate for temporary lockout of a bot (e.g., during a task)
        (helper_bot_is_free ?bot - helper_bot) ; bot is free (not carrying a box or helping a patient)
        (helper_bot_is_unlocked ?bot - helper_bot) ; predicate for temporary lockout of a bot (e.g., during a task)

        ; Worker bots
        (bot_owns_carrier ?bot - worker_bot ?carr - carrier) ; a bot owns a carrier
    ...
    (:durative-action move_worker_bot_alone
        :parameters (?bot - worker_bot ?from - location ?to - location)
        :duration (= ?duration 3)
        :condition (and
            (at start (worker_bot_is_unlocked ?bot)) ; unlock check
            
            ; location consistency
            (at start (at ?bot ?from))
            (over all (linked ?from ?to))
            ; the bot is free (carrier is empty / not helping a patient)
            (over all (worker_bot_is_free ?bot))
        )
        :effect (and
            (at start (not (worker_bot_is_unlocked ?bot))) ; LOCK (during execution)
            
            ; change location (bot)
            (at start (not (at ?bot ?from)))
            (at start (at ?bot ?to)) ; note: for movement, location is updated instantly.

            (at end (worker_bot_is_unlocked ?bot)) ; UNLOCK (after execution ends)
        )
    )

    (:durative-action move_helper_bot_alone
        :parameters (?bot - helper_bot ?from - location ?to - location)
        :duration (= ?duration 3)
        :condition (and
            (at start (helper_bot_is_unlocked ?bot)) ; unlock check
            
            ; location consistency
            (at start (at ?bot ?from))
            (over all (linked ?from ?to))
            ; the bot is free (carrier is empty / not helping a patient)
            (over all (helper_bot_is_free ?bot))
        )
        :effect (and
            (at start (not (helper_bot_is_unlocked ?bot))) ; LOCK (during execution)
            
            ; change location (bot)
            (at start (not (at ?bot ?from)))
            (at start (at ?bot ?to)) ; note: for movement, location is updated instantly.

            (at end (helper_bot_is_unlocked ?bot)) ; UNLOCK (after execution ends)
        )
    )
    ...
    (:durative-action pick_up_1st_box
        ...
        :effect (and
            ...
            ; bot becomes busy
            (at end (not (worker_bot_is_free ?bot)))
    ...
    (:durative-action pick_up_patient
        ...
        :condition (and
            ...
            ; helper bot is free
            (at start (helper_bot_is_free ?bot))
        )
        :effect (and
            (at start (not (helper_bot_is_unlocked ?bot))) ; LOCK (during execution)
            ...
            (at end (not (helper_bot_is_free ?bot)))
